...
export default function App() {
  ...
  const { containerStyle } = useMoveable()

  const chatPaneWidthClass = isWideChatPane ? 'w-[60vw]' : 'w-[40vw]'

  return (
    <div className="w-full h-full flex flex-col items-center justify-start gap-1 pt-2 fixed" style={containerStyle}>
      <Mainbar />

      <div
        className={`overflow-hidden transition-all duration-300 ease-in-out ${isChatPaneVisible ? 'max-h-[80vh] opacity-100' : 'max-h-0 opacity-0'} px-4 ${chatPaneWidthClass}`}
      >
